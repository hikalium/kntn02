...
char nkgwreturn[INPUT_LINE_SIZE];
int tlen;
...
void overwrite()
{
	...
	for(i=0; i<tlen; i++)
	{
		if(fixedStr[i])tbuf[i]=fixedStr[i];
	}

}
void find_seg_ofs(int segID)
{
	int check, i,lslen, seglen,j,bm;
	fprintf(stderr, "S[%d] = %s\n", segLenList[segID], segList[segID]);
	for(i = 0; i < tlen; i++)
	{	
		check=1;
		bm=0;
		const char *longstr = &fixedStr[i];
		lslen = tlen - i;
		...
		if(check)
			{
				strncpy(&nkgwreturn[i], segList[segID], segLenList[segID]);
			}
	}
}
void fillRestofXwithC()
{
	int i;
	for(i = 0; i < tlen; i++){
		if(nkgwreturn[i] != fixedStr[i] && fixedStr[i]!='x')nkgwreturn[i] = fixedStr[i];
		if(nkgwreturn[i]=='x')nkgwreturn[i] = 'c';
	}
}
